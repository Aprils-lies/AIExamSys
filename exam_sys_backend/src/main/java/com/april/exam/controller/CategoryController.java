...
import com.april.exam.service.CategoryService;
...
public class CategoryController {
    ...
    public Result<List<Category>> getCategories() {
        ...
        log.info("查询分类列表和题目数量业务成功！结果为：{}",categoryList);
    ...
    public Result<List<Category>> getCategoryTree() {
        List<Category> categoryList = categoryService.getCategoryTreeList();
        log.info("查询树状分类集合接口调用成功！数据为：{}",categoryList);
        return Result.success(categoryList);
    }
    ...
    public Result<Void> addCategory(@RequestBody Category category) {
        ...
        log.info("在{}父分类下，新增{}子分类成功！",category.getParentId(),category.getName());
        return Result.success("新增分类接口调用成功！");
    }
    ...
    public Result<Void> deleteCategory(
        ...
        return Result.success("删除分类接口调用成功！");
    }
} 