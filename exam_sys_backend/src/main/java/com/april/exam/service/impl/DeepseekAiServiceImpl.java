...
public class DeepseekAiServiceImpl implements DeepseekAiService {
    ...
    private String deepseekBaseUrl; // deepseek API基础地址
    
    @Value("${deepseek.api.model}")
    private String deepseekModel; // 使用的模型
        ...
        for (int attempt = 1; attempt <= maxRetries; attempt++) {
            try {
                ...
                Mono<String> responseMono = webClient.post()
                        ...
                        /*
                            Authorization: Bearer [API_KEY] 的格式含义
                            这是行业通用的 Bearer Token 认证规范（属于 HTTP 标准认证方式之一），格式拆解：
                            Authorization：HTTP 协议规定的 “认证头” 字段，告诉服务器 “这个请求需要验证身份”
                            Bearer：表示 “令牌类型”（可以理解为 “凭证类型”），说明后面跟着的是一个 “访问令牌”
                            [API_KEY]：你在 deepseek 平台申请的专属密钥（类似你的 “API 密码”）
                         */
